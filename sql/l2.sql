...
(
    ...
    `is_published`   tinyint(1) UNSIGNED                                   NOT NULL DEFAULT 0 COMMENT '是否发布（0=私密，1=公开）',
    `is_deleted`     tinyint(1) UNSIGNED                                   NOT NULL DEFAULT 0 COMMENT '逻辑删除标志（0=正常，1=已删除）',
...
(
    ...
    `is_active`   tinyint(3) UNSIGNED                                   NOT NULL DEFAULT 000 COMMENT '账户状态（0=正常，1=异常）',
    `is_deleted`  tinyint(3) UNSIGNED                                   NOT NULL DEFAULT 000 COMMENT '逻辑删除标志（0=正常，1=已删除）',
    `create_id`   bigint(0) UNSIGNED                                            NOT NULL COMMENT '创建人ID',
    `create_time` datetime(0)                                                   NOT NULL DEFAULT CURRENT_TIMESTAMP(0) COMMENT '创建时间',
    `update_id`   bigint(0) UNSIGNED                                            NOT NULL COMMENT '更新人ID',
    `update_time` datetime(0)                                                   NULL     DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP(0) COMMENT '更新时间',
