...
pub async fn list(pool: &PgPool, dto: &AuditLogListDto) -> R<PageResult<SysAuditLog>> {
    ...
    let mut count_builder = QueryBuilder::<Postgres>::new(
        "SELECT COUNT(*)::bigint FROM sys_audit_log WHERE 1=1",
    );
    ...
    let mut data_builder = QueryBuilder::<Postgres>::new(
        "SELECT * FROM sys_audit_log WHERE 1=1",
    );
    append_filters(&mut data_builder, dto);
    data_builder
        .push(" ORDER BY created_time DESC")
        .push(" LIMIT ")
...
pub async fn insert_log(
    ...
    let result = sqlx::query(
        "INSERT INTO sys_audit_log (id, user_id, user_name, method, path, params, status, duration_ms, ip, is_sensitive, created_time)
